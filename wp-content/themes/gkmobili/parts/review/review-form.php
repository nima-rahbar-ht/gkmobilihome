...
$comment_args = premmerce_get_comments_form_args();
...
if (get_option('woocommerce_enable_review_rating') === 'yes' && is_product()) {
    $comment_args['comment_field'] .= '<div class="form__field form__field--hor form__field--static form__field--review-rating"><div class="form__label">' . esc_html__('Your rating', 'saleszone') . '</div><div class="form__inner"><div class="star-voting"><div class="star-voting__wrap">';

    for ($i = 5; $i > 0; $i--) {
        $comment_args['comment_field'] .= '<input class="star-voting__input" id="star-voting-' . $i . '" type="radio" name="rating" value="' . $i . '">';
        $comment_args['comment_field'] .= '<label class="star-voting__ico" for="star-voting-' . $i . '" title="' . $i . ' ' . __('out of 5 stars', 'saleszone') . '">' . premmerce_get_svg('star') . '</label>';
    }
